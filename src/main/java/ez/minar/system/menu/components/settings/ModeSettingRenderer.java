...
public class ModeSettingRenderer implements SettingRenderer<ModeSetting> {
    private final Map<Setting, Float> animOffsets = new HashMap<>();
    ...
    public void render(DrawContext context, ModeSetting setting, SettingRendererContext rendererContext, float x, float y, float width, float scale, float opacity) {
        ...
        String actualMode = setting.getActiveMode();
        ...
        int activeIndex = Math.max(0, modes.indexOf(setting.getActiveMode()));

        float targetOffset = activeIndex * SettingRendererContext.SETTING_HEIGHT * scale;
        float currentOffset = animOffsets.getOrDefault(setting, targetOffset);

        if (currentOffset < targetOffset) {
            currentOffset = Math.min(targetOffset, currentOffset + delta * 250f * scale);
        } else if (currentOffset > targetOffset) {
            currentOffset = Math.max(targetOffset, currentOffset - delta * 250f * scale);
        }
        animOffsets.put(setting, currentOffset);

        float startY = y + SettingRendererContext.SETTING_HEIGHT * scale;
        
        // Render left vertical line background
        float totalHeight = modes.size() * SettingRendererContext.SETTING_HEIGHT * scale;
        ...
        // Animated Selection Box (Vertical Line)
        RenderUtil.rect(x, startY + currentOffset + 2f * scale, 1.5f * scale, SettingRendererContext.SETTING_HEIGHT * scale - 4f * scale, 0.75f * scale, rendererContext.withOpacity(ThemeManager.getThemeColor(), opacity));

        float optionY = startY;
        float textSize = 7.2f * scale;

        for (int i = 0; i < modes.size(); i++) {
            String option = modes.get(i);
            boolean isSelected = i == activeIndex;
            
            String key = System.identityHashCode(setting) + ":" + option;
            float currentProgress = animProgress.getOrDefault(key, isSelected ? 1f : 0f);
            
            if (isSelected && currentProgress < 1f) {
                currentProgress = Math.min(1f, currentProgress + delta * 8f);
            } else if (!isSelected && currentProgress > 0f) {
    ...
    public boolean click(ModeSetting setting, SettingRendererContext rendererContext, int button, float localX, float localY, float width) {
        ...
        if (localY >= SettingRendererContext.SETTING_HEIGHT) {
            ...
            if (index >= 0 && index < modes.size()) {
                setting.setMode(modes.get(index));
            }
            return true;
        }
    ...
    public float getHeight(ModeSetting setting, SettingRendererContext rendererContext) {
        return SettingRendererContext.SETTING_HEIGHT * (1f + setting.getModes().size());
    }
