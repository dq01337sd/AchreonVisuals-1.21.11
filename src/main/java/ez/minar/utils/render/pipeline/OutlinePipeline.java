...
public class OutlinePipeline {
    ...
    public static void init() {
        ...
        try {
            pipeline = RenderPipeline.builder()
                    .withLocation(Identifier.of("atheryx", "outline"))
                    .withVertexShader(Identifier.of("atheryx", "outline_vertex"))
                    .withFragmentShader(Identifier.of("atheryx", "outline_fragment"))
    ...
    public static void draw(Matrix4f matrix, float x, float y, float width, float height, float tl, float tr, float br,
                            float bl, float thickness,
                            float z, int... colors) {
        if (pipeline == null) {
            init();
        }
        ...
        buffer.putFloat(thickness); // uThickness
        buffer.putFloat(z); // uZ
        buffer.putFloat(0).putFloat(0); // _pad (vec2)
